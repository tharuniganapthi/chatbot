...
# Load spaCy model and other necessary components
...
# Function to extract skills and languages from PDF
def extract_skills_and_languages(text):
    doc = nlp(text)
    technical_skills = []
    languages = []
...
def read_pdf(uploaded_file):
    ...
    with BytesIO(uploaded_file.read()) as file:
        ...
        for page_num in range(len(pdf_reader.pages)):
            ...
            skills, _ = extract_skills_and_languages(text)
...
def start_interview(pdf_file_path, user_name, user_position):
    ...
    # Start the interview
    response = send_prompt_and_get_response(f"Hi there, My name is {user_name}")
    ...
    for i in range(100):
        ...
        if user_response is not None:
            response = send_prompt_and_get_response(user_response)
            st.write(response.text)
            speak_response(response.text)
        else:
